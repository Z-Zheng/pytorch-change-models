...
class MultiLoraLinear(nn.Linear, LoRALayer):
    def __init__(
        ...
        if r > 0:
            ...
            self.lora_B = nn.ParameterList([nn.Parameter(self.weight.new_zeros((r, in_features))) for _ in range(num_lora)])
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
        self.reset_parameters()
...
if __name__ == '__main__':
    # model = SAMEncoder(dict(
    #     # checkpoint='/Users/zhengzhuo/Desktop/sam_vit_b_01ec64.pth',
    #     # lora=dict(r=32, mode='attn'),
    #     vit_type='vit_b'
    # ))
    model = build_sam_vit_s()
    model.eval()

    # er.param_util.trainable_parameters(model)
    er.param_util.count_model_parameters(model)
    # with torch.no_grad():
