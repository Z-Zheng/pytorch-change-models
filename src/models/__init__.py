...
from .changestar_1xd import (
    ChangeStar1xdConfig,
    ChangeStar1xdModel,
    ChangeStar1xdForChangeDetection,
)
from .changemask import ChangeMaskConfig, ChangeMaskModel, ChangeMaskForChangeDetection
...
class ChangeDetectionModelFactory:
    ...
    def create_model(
        ...
        **kwargs,
        ...
        elif model_type == ModelType.CHANGEMASK:
            return ChangeDetectionModelFactory._create_changemask_model(config)
        else:
    ...
    @staticmethod
    def _create_changemask_model(
        config: Dict[str, Any],
    ) -> ChangeMaskForChangeDetection:
        """Create a ChangeMask model with the given configuration."""
        changemask_config = ChangeMaskConfig(**config)
        return ChangeMaskForChangeDetection(changemask_config)
    ...
    def list_available_models() -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "changemask": {
                "description": "U-Net style change detection with semantic segmentation",
                "strengths": [
                    "Semantic awareness",
                    "U-Net architecture",
                ...
                "config_options": [
                    "encoder_type (efficientnet-b0, resnet50, etc.)",
                    "encoder_weights",
                    "decoder_channels",
                    "temporal_interaction_type",
...
def create_changemask_model(**config) -> ChangeMaskForChangeDetection:
    """Create a ChangeMask model with the given configuration."""
    return ChangeDetectionModelFactory.create_model("changemask", config)
